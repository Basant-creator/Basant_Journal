...
const MARKS = {
  alpha: { box: { x: 16, y: 62, w: 16, h: 20 }, focus: [360, 640] as [number, number] },
  beta: { box: { x: 44, y: 58, w: 14, h: 22 }, focus: [760, 600] as [number, number] },
  gamma: { box: { x: 70, y: 64, w: 16, h: 20 }, focus: [1200, 660] as [number, number] },
};
...
export function SceneBench() {
  ...
  return (
    <div className={styles.bench}>
      ...
      <Scene width={1600} height={900} label="Scene engine test bench">
        <svg className={styles.art} viewBox="0 0 1600 900" aria-hidden="true">
          <SceneCamera target={camera ? { focus: MARKS[open].focus, zoom: 1.3 } : undefined}>
            {/* --- background ------------------------------------------- */}
            <SceneLayer depth={0} name="background">
              <rect x="0" y="0" width="1600" height="900" className={styles.sky} />
            ...
            <SceneLayer depth={6} name="objects">
              {ORDER.map((id, i) => (
                <g key={id} transform={`translate(${380 + i * 420} 640)`}>
                  <rect
                    className={open === id ? `${styles.mark} ${styles.markOpen}` : styles.mark}
                    x="-70"
                    y="-70"
                    width="140"
                    height="140"
